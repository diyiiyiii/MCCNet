...
def _calc_feat_flatten_mean_std(feat):
    assert (feat.size()[0]==3)  #三通道..?
    assert (isinstance(feat, jt.Float))   #这样可以吗...torch.FloatTensor
    feat_flatten = feat.view(3, -1)
    mean = feat_flatten.mean(dim=-1, keepdim=True)
    std = feat_flatten.std(dim=-1, keepdim=True)   #计算每个通道中的所有树的平均值..?
...
def _mat_sqrt(x):
    U, D, V =jt.svd(x)
    return matmul(matmul(U, D.pow(0.5).diag*()), V.t())  #这个函数在干嘛也不是很清楚...
