...
class FlatFolderDataset(Dataset):      #调用时输入为args.content_dir, content_tf 和 style  #本来继承的是troch.data.dataset,这两个一样吗？  #！应该用ImageFolder！/Dataset
...
#sampler的作用就是每次都取随机的样本(重复也是可以的..?）咋整呢...怎么才能放到iter里面还能每次next调用呢..
    ...
    def __len__(self):   #返回目录中的文件个数
        return len(self.path)

    def name(self):
        return 'FloatFolderDataset'
...
#USE_CUDA = jt._get_use_cuda()  ##这两句再搜索一下吧...
#device = jt.device("cuda:0" if USE_CUDA else "cpu")  #直接去掉好像也不太行  #有了直接去了就行了

if not os.path.exists(args.save_dir):  #文件不存在就创建
...
#vgg.load_state_dict(jt.load(args.vgg)) #加载训练好的vgg数据 这个方法不可行
vgg = nn.Sequential(*list(vgg.children())[:44])
with jt.no_grad():  #这条语句参数不需要更新，都是预训练好的
    network = net.Net(vgg, decoder)    #Net是包装了的网络，vgg为encoder，decoder为decoder
network.train() #将当前模块设置为train模式
...
optimizer = jt.optim.Adam([    #带有动量的RMSprop  #但这个参数是在干什么不是特别清楚..   #.module去掉
    {'params':network.decoder.parameters()},
    {'params':network.mcc_module.parameters()}
]   ,lr=args.lr)

for i in tqdm(range(args.max_iter)):   #进度条...
...
#          ,"-l1:",l_identity1.sum().cpu().detach().numpy(), "-l2:", l_identity2.sum().cpu().detach().numpy(), loss_tv.sum().cpu().detach().numpy())
    ...
    ### 最后测试一下需不需要将.cpu().detach()去掉
    #.cpu().detach().numpy()结果是将数据取出来
    Ics = network.Icstest[0]
    writer.add_image('Ics',Ics.numpy(),i+1)

    optimizer.zero_grad() #梯度置为0，不然会重新计算
    ...
    if (i + 1) % args.save_model_interval == 0 or (i + 1) == args.max_iter:
        ...
        jt.save(state_dict,
                '{:s}/decoder_iter_{:d}.pth'.format(args.save_dir, i+1))  #周期性的保存权重  可是decoder不是固定的吗...

        state_dict = network.mcc_module.state_dict()
        jt.save(state_dict,
                '{:s}/mcc_module_iter_{:d}.pth'.format(args.save_dir, i+1 ))
