...
class Self_Attention_Module(nn.Module):
    def __init__(self, in_dim):
        ...
        #self.SANC1=Content_SANet(in_dim)
        #self.SANS1=Style_SANet(in_dim)
        
        # self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        # self.merge_conv_pad = nn.ReflectionPad2d((1, 1, 1, 1))
        # self.merge_conv = nn.Conv2d(in_dim, in_dim, (3, 3))
    def forward(self, content_feats, style_feats):
        # content_feat_4 = self.SANC1(content_feats[-1])
        # style_feat_4 = self.SANS1(style_feats[-1])
        # Fcsc_5 = self.SAN1(content_feat_4, style_feat_4)
        # Fcsc_5_up=self.upsample(Fcsc_5)
        # content_feat_5 = self.SANC2(content_feats[-2])
        # style_feat_5 = self.SANS2(style_feats[-2])
        content_feat_5 = content_feats[-2]
        style_feat_5 = style_feats[-2]
        Fcsc_4 = self.SAN2(content_feat_5, style_feat_5)
...
class Net(nn.Module):
    ...
    def forward(self, content, style):
        ...
        #Ics_T = self.decoder(self.sa_module(content_feats_T, style_feats)).permute(0,1,3,2)
        Ics_N = self.decoder(self.sa_module(content_feats_N, style_feats))
        loss_noise = self.calc_content_loss(Ics_N,Ics)
        # Isc = self.decoder(self.sa_module(style_feats, content_feats))
        # Isc_feats = self.encode_with_intermediate(Isc)
        # # Content loss
        # c_loss_c = self.calc_content_loss(normal(Isc_feats[-1]), normal(style_feats[-1]))+self.calc_content_loss(normal(Isc_feats[-2]), normal(style_feats[-2]))
        # # Style loss
        # c_loss_s = self.calc_style_loss(Isc_feats[0], content_feats[0])
        # for i in range(1, 5):
        #     c_loss_s += self.calc_style_loss(Isc_feats[i], content_feats[i])
